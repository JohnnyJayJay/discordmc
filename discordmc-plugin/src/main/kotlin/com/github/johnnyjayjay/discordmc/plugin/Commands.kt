...
object Commands {
    ...
    fun discord(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        checkPermission(sender, "discordmc.message.write") {
            if (args.isEmpty())
                return false
    ...
    fun linkInfo(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        checkPermission(sender, "discordmc.info") {
            if (args.isNotEmpty())
                return false
    ...
    private inline fun checkPermission(
        sender: CommandSender,
        vararg permissions: String = emptyArray(),
        body: () -> Unit
    ) {
        if (sender.isOp || permissions.all(sender::hasPermission)) {
            body()
        } else {
